...
export function SidePanel({
  ...
  return (
    <aside className="flex h-full min-h-[520px] w-full max-w-xl flex-col overflow-hidden rounded-3xl border border-white/8 bg-[#3c3a37] shadow-[0_10px_30px_rgba(0,0,0,0.22)]">
      <header className="border-b border-white/10 px-4 py-3">
        <p className="text-sm font-semibold text-[#f5f5f5]">Analyse</p>
        <p className="text-xs text-[rgba(255,255,255,0.72)]">{status}</p>
      </header>

      <section className="border-b border-white/10 px-4 py-3 text-xs text-[#f5f5f5]">
        <div className="mb-2 flex items-center justify-between">
          <span>Éval. {formatEvaluation(engineOutput.evaluation)}</span>
          <span>{STATUS_LABELS[engineStatus]}</span>
        </div>
        <div className="space-y-1">
          {topLines.length === 0 && <p className="text-[rgba(255,255,255,0.72)]">En attente des variantes moteur…</p>}
          {topLines.map((line, index) => (
            <div key={`pv-${index}`} className="flex items-start gap-2 rounded-md bg-black/10 px-2 py-1">
              <span className="min-w-10 text-[rgba(255,255,255,0.72)]">#{index + 1}</span>
              <span className="line-clamp-1 flex-1 text-[#f5f5f5]">{line.pv?.line ?? '—'}</span>
              <span className="text-[rgba(255,255,255,0.72)]">{formatEvaluation(line.evaluation)}</span>
            </div>
          ))}
        </div>
      ...
      <section className="border-b border-white/10 px-4 py-3">
        <div className="mb-2 flex items-center justify-between text-xs text-[rgba(255,255,255,0.72)]">
          <span>Coups</span>
          <span>
            Position {currentIndex}/{Math.max(positionsCount - 1, 0)}
          </span>
        </div>
        <div className="grid grid-cols-4 gap-2 text-xs text-[#f5f5f5]">
          <button type="button" onClick={() => onNavigate('start')} className="rounded-lg border border-white/10 px-2 py-1 hover:bg-white/5">⏮</button>
          <button type="button" onClick={() => onNavigate('prev')} className="rounded-lg border border-white/10 px-2 py-1 hover:bg-white/5">←</button>
          <button type="button" onClick={() => onNavigate('next')} className="rounded-lg border border-white/10 px-2 py-1 hover:bg-white/5">→</button>
          <button type="button" onClick={() => onNavigate('end')} className="rounded-lg border border-white/10 px-2 py-1 hover:bg-white/5">⏭</button>
        </div>
      </section>
