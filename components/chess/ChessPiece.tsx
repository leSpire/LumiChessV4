...
import { getPieceAssetUrl } from '@/lib/pieceThemes';
...
export function ChessPiece({ type, color, themeId, isDragging }: ChessPieceProps) {
  ...
  useEffect(() => setErrored(false), [themeId, color, type]);
    ...
    >
      {pieceUrl ? (
        <img
          ...
          className={clsx(baseClasses, 'select-none object-contain')}
        />
      ) : (
        <ClassicPiece type={type} color={color} />
      )}
    </div>
  );
}
